...
def estimator(
    ...
    if (
        ...
        and initials_dict["initial_type"] == "optimal"
        and initials_dict["initial_arima_estimate"]
        and B is None
    ):
        ...  # will add later!
    ...
    if general_dict["loss"] in ["LASSO", "RIDGE"]:
        ...
        general_dict["y_denominator"] = max(
    ...
    opt = nlopt.opt(
        nlopt.LN_NELDERMEAD, len(B)
    )  # Use BOBYQA algorithm which is better for this type of problem
...
def selector(
    ...
    if model_type_dict["models_pool"] is None:
        ...
        while check:
            ...
            results[i - 1]["IC"] = ic_function(
                general_dict["ic"],
                loglik=results[i - 1]["adam_estimated"]["log_lik_adam_value"],
            )
            ...
            if j > 1:
                ...
                if results[best_i - 1]["IC"] <= results[i - 1]["IC"]:
                    ...
                    # Otherwise we checked trend
                        ...
                        pool_trends = results[best_j - 1]["model_type_dict"][
                            "trend_type"
                        ]
                        check_trend = False
                else:
    ...
    while j < models_number:
        ...
        # this need further itteration on how to return outputs
        results[j - 1]["IC"] = ic_function(
            general_dict["ic"],
            loglik=results[j - 1]["adam_estimated"]["log_lik_adam_value"],
        )
...
def preparator_old(
    ...
    if phi_dict["phi_estimate"]:
        phi_dict["phi"] = (
            adam_estimated["B"],
            [next(i for i, v in enumerate(B.keys()) if v == "phi")],
        )

    # Write down the initials in the recent profile
    profiles_dict["profiles_recent_table"][:] = matrices_dict["mat_vt"][
        :, : lags_dict["lags_model_max"]
    ]
    profiles_dict["profiles_recent_initial"] = matrices_dict["mat_vt"][
        :, : lags_dict["lags_model_max"]
    ].copy()
    ...
    vec_g = np.asfortranarray(
        matrices_dict["vec_g"], dtype=np.float64
    ...
    profiles_recent_table = np.asfortranarray(
        profiles_dict["profiles_recent_table"], dtype=np.float64
    )
    ...
    if occurrence_dict["occurrence"] == "provided":
        y_fitted[~occurrence_dict["ot_logical"]] = (
            y_fitted[~occurrence_dict["ot_logical"]]
    ...
    if (
        general_dict["distribution_new"] in ["dalaplace", "dgnorm", "dlgnorm", "dt"]
        and initials_checked["other_parameter_estimate"]
    ):
