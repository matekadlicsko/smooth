            ...
            if initials_checked["initial_estimate"]:
                ...
                if model_is_seasonal:
                    if obs_nonzero >= lags_model_max * 2:
                        ...
                        decomposition_type = (
                            "multiplicative"
                            if any(x == "M" for x in [e_type, s_type])
                            else "additive"
                        )
                        ...
                        if initials_checked["initial_level_estimate"]:
                            ...
                            if explanatory_checked["xreg_model"]:
                                if e_type == "A":
                                    ...
                                    mat_vt[j, 0:lags_model_max] /= np.exp(
                                        np.dot(
                                            explanatory_checked["xreg_model_initials"][
                                                1
                                            ]["initial_xreg"],
                                            explanatory_checked["xreg_data"][0],
                                        )
                            ...
                            mat_vt[j, 0:lags_model_max] = initials_checked[
                                "initial_level"
                            ]
                        j += 1
                        # If trend is needed
                            ...
                            if initials_checked["initial_trend_estimate"]:
                                if t_type == "A" and s_type == "M":
                                    mat_vt[j, 0:lags_model_max] = (
                                        np.prod(y_decomposition["initial"])
                                        - y_decomposition["initial"][0]
                                    )
                                    ...
                                    # trend
                                    mat_vt[j, 0:lags_model_max] = y_decomposition[
                                        "initial"
                                    ][
                                        1
                                    ]  # This is a failsafe for multiplicative trend models, so that the thing does not explode
                                if t_type == "M" and np.any(
                                    mat_vt[j, 0:lags_model_max] > 1.1
                                ):
                                    mat_vt[j, 0:lags_model_max] = 1
                                # This is a failsafe for multiplicative trend models, so that the thing does not explode
                                if t_type == "M" and np.any(
                                    mat_vt[0, 0:lags_model_max] < 0
                                ):
                                    mat_vt[0, 0:lags_model_max] = y_in_sample[
                                        ot_logical
                                    ][0]
                            else:
                                mat_vt[j, 0:lags_model_max] = initials_checked[
                        ...
                        if (
                            ...
                            for i in range(components_number_ets_seasonal):
                                if initials_checked["initial_seasonal_estimate"]:
                                    ...
                                    mat_vt[i + j, 0 : lags_model[i + j]] = (
                                        initials_checked["initial_seasonal"][i]
                                    )
                        ...
                        # level
                        if initials_checked["initial_level_estimate"]:
                            mat_vt[j, 0:lags_model_max] = np.mean(
                                y_in_sample[0:lags_model_max]
                            )
                            if explanatory_checked["xreg_model"]:
                                if e_type == "A":
                                    mat_vt[j, 0:lags_model_max] -= np.dot(
                        ...
                        if model_is_trendy:
                            ...
                            if t_type == "A":
                                mat_vt[j - 1, 0:lags_model_max] = (
                                    mat_vt[j - 1, 0] - mat_vt[j, 0] * lags_model_max
                                )
                            elif t_type == "M":
                                mat_vt[j - 1, 0:lags_model_max] = (
                                    mat_vt[j - 1, 0] / mat_vt[j, 0] ** lags_model_max
                            ...
                            for i in range(components_number_ets_seasonal):
                                if initials_checked["initial_seasonal_estimate"]:
                                    mat_vt[i + j - 1, 0 : lags_model[i + j - 1]] = (
                                        y_in_sample[0 : lags_model[i + j - 1]]
                                        - mat_vt[0, 0]
                                    )
                                    # Renormalise the initial seasons
                                    mat_vt[i + j - 1, 0 : lags_model[i + j - 1]] -= (
                    ...
                    if initials_checked["initial_level_estimate"]:
                        mat_vt[0, :lags_model_max] = np.mean(
                            y_in_sample[
                                : max(lags_model_max, int(np.ceil(obs_in_sample * 0.2)))
                            ]
                        )
                    else:
                    ...
                    if model_is_trendy:
                        if initials_checked["initial_trend_estimate"]:
                            ...
                            mat_vt[1, 0:lags_model_max] = initials_checked[
                                "initial_trend"
                            ]
...
def initialiser(
    ...
    persistence_estimate_vector = [
        ...
        and any(persistence_checked["persistence_seasonal_estimate"]),
    ]
        ...
        if persistence_checked["persistence_estimate"] and any(
            ...
            if any(
                ...
                if (
                    ...
                    B[j : j + sum(persistence_estimate_vector)] = [0.01, 0] + [
                        0
                    ...
                    B[j : j + sum(persistence_estimate_vector)] = [0, 0] + [
                        0
                    ] * components_dict["components_number_ets_seasonal"]
                elif (
                    model_type_dict["error_type"] == "M"
                    ...
                    if initials_checked["initial_type"] in ["complete", "backcasting"]:
                        B[j : j + sum(persistence_estimate_vector)] = [0.1, 0] + [
                            0.3
                        ] * components_dict["components_number_ets_seasonal"]
                    else:
                    ...
                    if model_type_dict["model_is_seasonal"]:
                        initial_values.extend(
                            [0.3] * components_dict["components_number_ets_seasonal"]
                        )
    ...
    if arima_checked["arima_model"]:
        if any([arima_checked["ar_estimate"], arima_checked["ma_estimate"]]):
            ...
            if not (
                model_type_dict["ets_model"] or all(arima_checked["i_orders"] == 0)
            ):
                ...
                if all(np.array(lags_dict["lags"]) <= 1):
                    if arima_checked["ma_required"] and arima_checked["ma_estimate"]:
                        acf_values[
                            : min(
                                sum(arima_checked["ma_orders"] * lags_dict["lags"]),
                                len(y_differenced) - 1,
                            )
                        ] = calculate_acf(
                            y_differenced,
                            nlags=max(
                                1, sum(arima_checked["ma_orders"] * lags_dict["lags"])
                            ),
                        )[1:]
                    if arima_checked["ar_required"] and arima_checked["ar_estimate"]:
                        pacf_values[
                            ...
                            y_differenced,
                            nlags=max(
                                1, sum(arima_checked["ar_orders"] * lags_dict["lags"])
                            ),
                        )
        ...
        if model_type_dict["ets_model"]:
            ...
            if model_type_dict["model_is_seasonal"]:
                if any(persistence_checked["persistence_seasonal_estimate"]):
                    ...
                    matrices_dict["vec_g"][seasonal_indices] = B[
                        j : j
                        + sum(persistence_checked["persistence_seasonal_estimate"])
                    ]
                    j += sum(persistence_checked["persistence_seasonal_estimate"])
        ...
        try:
            arima_polynomials = adam_polynomialiser(
                B[
                    j : j
                    + sum(
                        np.array(arima_checked["ar_orders"])
                        * arima_checked["ar_estimate"]
                        + np.array(arima_checked["ma_orders"])
                        * arima_checked["ma_estimate"]
                    )
                ],
                arima_checked["ar_orders"],
            ...
            if len(arima_checked["non_zero_ari"]) > 0:
                matrices_dict["mat_f"][
                    ...
                    components_dict["components_number_ets"] : components_dict[
                        "components_number_ets"
                    ]
                    + components_dict["components_number_arima"]
                    + constants_checked["constant_estimate"],
            ...
            if len(arima_checked["non_zero_ma"]) > 0:
                matrices_dict["vec_g"][
                    components_dict["components_number_ets"]
                    + arima_checked["non_zero_ma"][:, 1]
                ] += arima_polynomials["maPolynomial"][
                    arima_checked["non_zero_ma"][:, 0]
                ]
        except Exception as e:
            print(f"DEBUG - Error in ARIMA processing: {e}")

        j += sum(
            np.array(arima_checked["ar_orders"]) * arima_checked["ar_estimate"]
...
def adam_profile_creator_old(
    ...
    profiles_recent_table = np.zeros((len(lags_model_all), lags_model_max))
    index_lookup_table = np.ones((len(lags_model_all), obs_all + lags_model_max - 1))
    profile_indices = (
        np.arange(1, lags_model_max * len(lags_model_all) + 1)
        .reshape(-1, len(lags_model_all))
        .T
    ...
    for i, lag in enumerate(lags_model_all):
        ...
        # to cover 'obsAll' observations.
        # '- 1' at the end adjusts these values to Python's zero-based indexing.
        index_lookup_table[i, lags_model_max - 1 : (lags_model_max + obs_all - 1)] = (
            np.tile(
                profile_indices[i, : lags_model_all[i]],
                int(np.ceil(obs_all / lags_model_all[i])),
            )[0:obs_all]
...
def architector(
    ...
    xreg_model = (
        False if explanatory_checked is None else explanatory_checked["xreg_model"]
    )
    xreg_number = (
        0 if explanatory_checked is None else explanatory_checked.get("xreg_number", 0)
    )

    constant_required = (
        False if constants_checked is None else constants_checked["constant_required"]
    )
    ...
    if ets_model:
        ...
        # Make lags (1, 1)
        lags_model = [1, 1] if model_is_trendy else [1]
        components_names_ETS = ["level", "trend"] if model_is_trendy else ["level"]

        model_is_seasonal = S_type != "N"
        if model_is_seasonal:
            # If the lags are for the non-seasonal model
            lags_model.extend(lags_model_seasonal)
            components_number_ETS_seasonal = len(lags_model_seasonal)
            if components_number_ETS_seasonal > 1:
                components_names_ETS.extend(
                    [f"seasonal{i + 1}" for i in range(components_number_ETS_seasonal)]
                )
            else:
    ...
    if arima_model:
        ...
        components_names_ARIMA = [
            f"arima{i + 1}" for i in range(components_number_ARIMA)
        ]
    ...
    # print('lags_model_max:', lags_model_max)
    # print('observations_dict["obs_in_sample"] + lags_model_max:', observations_dict["obs_in_sample"] + lags_model_max)
    ...
    model_type_dict.update(
        {"model_is_trendy": model_is_trendy, "model_is_seasonal": model_is_seasonal}
    )
    ...
    lags = lags_model_all
    new_lags_length = len(lags)
    new_lags_max = max(lags) if lags else 1
    ...
    lags_dict.update(
        {
            ...
            "lags_model_seasonal": lags_model_seasonal,
            "lags_length": new_lags_length,
            "max_lag": new_lags_max,
        }
    )
