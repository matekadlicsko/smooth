...
def _check_model_composition(model_str, allow_multiplicative=True, silent=False):
    ...
    if model_do in ["select", "combine"]:
        models_pool = _generate_models_pool(
            error_type, trend_type, season_type, allow_multiplicative, silent
        )
...
def _generate_models_pool(
    ...
    # Handle standard selection case
        ...
        season_options = (
            ["N", "A", "M"]
            if season_type != "N" and allow_multiplicative
            else ["N", "A"]
            if season_type != "N"
            else ["N"]
        )
...
def _check_ets_model(model, distribution, data, silent=False):
    ...
    if data_min <= 0 and distribution in [
        "dlnorm",
        "dgamma",
        "dinvgauss",
        "dllaplace",
    ...
    if (
        not allow_multiplicative
        and not silent
        and any(x == "M" for x in (error_type, trend_type, season_type))
    ):
        _warn(
            "Your data contains non-positive values, so the ETS might break for multiplicative components."
        )
...
def _check_arima(orders, validated_lags, silent=False):
    ...
    # Get non-zero values
    result["non_zero_ari"] = sorted(
        set([x for sublist in ari_values for x in sublist if x != 0])
    )
    result["non_zero_ma"] = sorted(
        set([x for sublist in ma_values for x in sublist if x != 0])
    )

    # Combine and sort unique lags for ARIMA components
    result["lags_model_arima"] = sorted(
        set(result["non_zero_ari"] + result["non_zero_ma"])
    )
        ...
        if result["ma_required"]:
            for lag in validated_lags:
                for i in range(max(ma_orders)):
                    result["arma_parameters"].append(
                        {"name": f"theta{i + 1}[{lag}]", "value": None}
                    )

    # Warning for high frequency data
    if max(validated_lags) >= 24 and not silent:
        _warn(
            "The estimation of ARIMA model with large lags might take time. Consider initial='backcasting' or simpler orders.",
            silent,
        )
...
def _check_outliers(outliers_mode, silent=False):
    ...
    if outliers_mode not in choices:
        _warn(
            f"outliers='{outliers_mode}' not recognized. Switching to 'ignore'.", silent
        )
    ...
    if damped:
        if phi is None:
            ...
            try:
                ...
                _warn(
                    f"Provided value of phi is invalid. Using 0.95 as a guess, will estimate it.",
                    silent,
                )
                phi_val = 0.95
                phi_est = True
...
def _check_persistence(
    ...
    if isinstance(persistence, dict):
        ...
        if "level" in persistence or "alpha" in persistence:
            result["persistence_level"] = persistence.get(
                "level", persistence.get("alpha")
            )
            result["persistence_level_estimate"] = False

        if "trend" in persistence or "beta" in persistence:
            result["persistence_trend"] = persistence.get(
                "trend", persistence.get("beta")
            )
        ...
        if "xreg" in persistence or "delta" in persistence:
            result["persistence_xreg"] = persistence.get(
                "xreg", persistence.get("delta")
            )
        ...
        _warn(
            "Persistence is not numeric/list/dict. We'll estimate it instead.", silent
        )
        return result
...
def _check_constant(constant, silent=False):
    ...
    _warn(
        f"The parameter 'constant' can only be bool or numeric. Found '{constant}'. Switching to FALSE",
        silent,
    )
    return {
...
def _initialize_estimation_params(
    ...
    if loss not in ["LASSO", "RIDGE"] or lambda_param != 1:
        return {"lambda": lambda_param, "persistence_params": None, "arma_params": None}
    ...
    if arima_info["arima_model"]:
        ...
        lags = sorted(
            set([1] + (arima_info.get("lags", []) or []))
        )  # Ensure we have lags

        # Initialize ARMA parameters
        arma_params = {"ar_estimate": False, "ma_estimate": False, "parameters": []}

        # Build ARMA parameters list with proper naming
        for lag in lags:
            # Add AR parameters (all set to 1)
            ar_count = (
                ar_orders[0] if ar_orders else 0
            )  # Simplified - might need adjustment
            for i in range(ar_count):
                arma_params["parameters"].append(
                    {"name": f"phi{i + 1}[{lag}]", "value": 1}
                )

            # Add MA parameters (all set to 0)
            ma_count = (
                ma_orders[0] if ma_orders else 0
            )  # Simplified - might need adjustment
            for i in range(ma_count):
                arma_params["parameters"].append(
                    {"name": f"theta{i + 1}[{lag}]", "value": 0}
                )
...
def _organize_components_info(ets_info, arima_info, lags_model_seasonal):
    ...
    return {
        ...
        "components_names_arima": [
            f"ARIMAState{i + 1}"
            for i in range(len(arima_info.get("lags_model_arima", [])))
        ],
        # Seasonal info
        "lags_model_seasonal": lags_model_seasonal,
        # Total components
        "components_number_total": (
            components_number_ets + len(arima_info.get("lags_model_arima", []))
        ),
    }
...
def _organize_lags_info(
    ...
    if xreg_model:
        ...
        lags_model_all = sorted(
            set(lags_model + (lags_model_arima if lags_model_arima else []))
        )
    ...
    if isinstance(occurrence, (int, float, np.number)):
        if all(occurrence == 1):
            ...
            if any(o < 0 or o > 1 for o in np.atleast_1d(occurrence)):
                _warn(
                    "Parameter 'occurrence' should contain values between zero and one.\nConverting to appropriate vector."
                )
            ...
            if h > 0:
                ...
                if len(p_forecast) > h:
                    ...
                    p_forecast = np.append(
                        p_forecast, np.repeat(p_forecast[-1], h - len(p_forecast))
                    )
            else:
...
def parameters_checker(
    ...
    if ets_model and (obs_nonzero < 3):
        ...
        _warn(
            "Not enough of non-zero observations for a complicated ETS model. Switching to 'ANN'.",
            silent,
        )
    ...
    allow_multiplicative = not (
        (any(y <= 0 for y in data_values if not pd.isna(y)) and not occurrence_model)
        or (occurrence_model and any(y < 0 for y in data_values if not pd.isna(y)))
    )
    ...
    observations_dict = {
        ...
        "y_in_sample_index": ot_info.get(
            "y_in_sample_index", None
        ),  # Add the index to observations_dict
...
def _calculate_ot_logical(
    ...
    if holdout:
        y_forecast_start = (
            data.index[obs_in_sample] if hasattr(data, "index") else obs_in_sample + 1
        )
    else:
        if hasattr(data, "index"):
            # Get the last observation and add one time unit
            y_forecast_start = data.index[obs_in_sample - 1] + (
                data.index[obs_in_sample - 1] - data.index[obs_in_sample - 2]
            )
        else:
