...
def calculate_multistep_loss(loss, adamErrors, obsInSample, horizon):
    if loss == "MSEh":
        ...
        return np.sum(
            np.log(np.sum(np.abs(adamErrors), axis=0) / (obsInSample - horizon))
        )
    elif loss == "MACE":
        return np.sum(np.abs(np.sum(adamErrors, axis=1))) / (obsInSample - horizon)
    elif loss == "HAMh":
        return np.sum(np.sqrt(np.abs(adamErrors[:, horizon - 1]))) / (
            obsInSample - horizon
        )
    elif loss == "THAM":
        return np.sum(
            np.sum(np.sqrt(np.abs(adamErrors)), axis=0) / (obsInSample - horizon)
        )
    elif loss == "GTHAM":
        return np.sum(
            np.log(
        ...
        return np.sum(np.sqrt(np.abs(np.sum(adamErrors, axis=1)))) / (
            obsInSample - horizon
        )
    elif loss == "GPL":
        return np.log(
            np.linalg.det(adamErrors.T @ adamErrors / (obsInSample - horizon))
        )
    else:
        return 0
