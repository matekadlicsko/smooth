...
def CF(
    ...
    if bounds == "usual":
        if arima_checked["arima_model"] and any(
            ...
            if (
                ...
                and sum(-adamElements["arimaPolynomials"]["arPolynomial"][1:]) >= 1
            ):
                arPolynomialMatrix[:, 0] = -adamElements["arimaPolynomials"][
                    "arPolynomial"
                ][1:]
            ...
            if (
                ...
                and sum(adamElements["arimaPolynomials"]["maPolynomial"][1:]) >= 1
            ):
                maPolynomialMatrix[:, 0] = adamElements["arimaPolynomials"][
                    "maPolynomial"
                ][1:]
        ...
        if model_type_dict["ets_model"]:
            ...
            if any(
                adamElements["vec_g"][: components_dict["components_number_ets"]] > 1
            ) or any(
                adamElements["vec_g"][: components_dict["components_number_ets"]] < 0
            ):
                return 1e100
            ...
            if phi_dict["phi_estimate"] and (
                adamElements["mat_f"][1, 1] > 1 or adamElements["mat_f"][1, 1] < 0
            ):
                return 1e100
        ...
        if arima_checked["arima_model"]:
            if arima_checked["ar_estimate"] and (
                ...
                or sum(-adamElements["arimaPolynomials"]["arPolynomial"][1:]) < 0
            ):
                arPolynomialMatrix[:, 0] = -adamElements["arimaPolynomials"][
                    "arPolynomial"
                ][1:]
        ...
        if model_type_dict["ets_model"] or arima_checked["arima_model"]:
            if explanatory_checked["xreg_model"]:
                ...
                if model_type_dict["ets_model"] or (
                    ...
                    and (
                        sum(adamElements["arimaPolynomials"]["maPolynomial"][1:]) >= 1
                        or sum(adamElements["arimaPolynomials"]["maPolynomial"][1:]) < 0
                    )
                ):
                    eigenValues = np.abs(
                        eigvals(
    ...
    profile_dict["profiles_recent_table"][:] = adamElements["mat_vt"][
    ...
    if not general["multisteps"]:
        if general["loss"] == "likelihood":
            ...
            CFValue = (
                np.sum(adam_fitted["errors"] ** 2) / observations_dict["obs_in_sample"]
            )
        elif general["loss"] == "MAE":
            CFValue = (
                np.sum(np.abs(adam_fitted["errors"]))
                / observations_dict["obs_in_sample"]
            )
            ...
            CFValue = general["loss_function"](
                actual=observations_dict["y_in_sample"],
...
def log_Lik_ADAM(
    ...
    if not multisteps:
        ...
        if model_type_dict["ets_model"] and model_type_dict["error_type"] == "M":
            ...
            # Write down the initials in the recent profile
            profile_dict["profiles_recent_table"][:] = adam_elements["matVt"][
                :, : lags_dict["lags_model_max"]
            ]

            # Fit the model again to extract the fitted values
            adam_fitted = adam_fitter(
                adam_elements["mat_vt"],
