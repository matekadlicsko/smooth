...
class Adam(object):
    ...
    def fit(
        ...
        if self.model_type_dict["model_do"] == "estimate":
            ...
            if (
                self.general["loss"] in ["LASSO", "RIDGE"]
                and self.general["lambda"] == 1
            ):
                ...
                if self.model_type_dict["arima_model"]:
                    ...
                    for i, lag in enumerate(self.lags_dict["lags"]):
                        if self.arima_results["ar_orders"][i] > 0:
                            arma_parameters.extend(
                                [1] * self.arima_results["ar_orders"][i]
                            )
                            j += self.arima_results["ar_orders"][i]
                        if self.arima_results["ma_orders"][i] > 0:
                            arma_parameters.extend(
                                [0] * self.arima_results["ma_orders"][i]
                            )
            ...
            if "parameters_number" not in self.general:
                self.general["parameters_number"] = self.params_info[
                    "parameters_number"
                ]
            ...
            for i in range(len(self.adam_selected["results"])):
                ...
                self.adam_selected["results"][i]["parameters_number"] = self.general[
                    "parameters_number"
                ]
